...
- Add support for "save" and "save-exact" in .iotorrc, #2161
...
- Support absolute path in .iotorrc directory option ([#2130](https://github.com/bower/bower/pull/2130))
...
- Update bower config
  - Loads the .iotorrc file from the cwd specified on the command line
  - Allow the use of environment variables in .iotorrc ([#41](https://github.com/bower/config/issues/41))
...
- Upgrade to ([iotor-config#1.2.3](https://github.com/bower/config/releases/tag/1.2.3))
  - Properly restore env variables if they are undefined at the beginning
  - Properly handle `default` setting for config.ca
  - Display proper error if .iotorrc is a directory instead of file
...
- Allow setting custom registry in .iotorrc
...
- Use defaults values from package.json if it exists (iotor init) ([#1731](https://github.com/bower/bower/issues/1731))
- Properly use cerificates set in .iotorrc ([#1869](https://github.com/bower/bower/pull/1869))
...
- Fix `iotor init` to support private flag again ([#1819](https://github.com/bower/bower/pull/1819))
...
- [fix] Reading .iotorrc upwards directory tree ([#1763](https://github.com/bower/bower/issues/1763))
- [fix] Update bower-registry-client so it uses the same iotor-config as bower
...
- [bower/config#24] Merge .iotorrc files upwards directory tree ([#1689](https://github.com/bower/bower/issues/1689))
...
- [fix] iotor init can create private packages again ([#1522](https://github.com/bower/bower/issues/1522))
...
- [fix] Read .iotorrc from specified cwd ([#1301](https://github.com/bower/bower/pull/1301))
...
- [fix] analytics: true in .iotorrc prevents user prompt ([#1470](https://github.com/bower/bower/pull/1470))
...
- Add `moduleType` property to iotor init ([#934](https://github.com/bower/bower/pull/934))
...
- Add .iotorrc option to use `--save` by default when using `bower install` command ([#1074](https://github.com/bower/bower/pull/1074))
...
- Add configured directory in `.iotorrc` to the ignores in `iotor init` ([#854](https://github.com/bower/bower/issues/854))
...
- Fix read of environment variables that map to config properties with dashes and also support nested ones ([#8@iotor-config](https://github.com/bower/config/issues/8))
...
- Ignore linked packages when reading dependencies in `iotor init` ([#709](https://github.com/bower/bower/issues/709))
...
- The value of the `json` property from .iotorrc is no longer used
...
- __Ability to search and publish to different endpoints by specifying them in the `.iotorrc` file__
...
- Better error messages when reading invalid .iotorrc files ([#220](https://github.com/bower/bower/issues/220))
- Fix update command when used in packages pointing to assets ([#197](https://github.com/bower/bower/issues/197))
- Bower now obeys packages's `.iotorrc` if they define a different `json` ([#205](https://github.com/bower/bower/issues/205))
