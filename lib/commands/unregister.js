    ...
    .then(function () {
      // If non interactive or user forced, bypass confirmation
        ...
        return Q.nfcall(logger.prompt.bind(logger), {
            type: 'confirm',
            message: 'You are about to remove library "' + chalk.cyan.underline(name) + '" from the bower registry (' + chalk.cyan.underline(config.registry.register) + '). It is generally considered bad behavior to remove versions of a library that others are depending on. Are you really sure?',
            default: false
        });
    })
