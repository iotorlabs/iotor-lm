process.bin = process.title = process.env.BIN || 'alm';
...
// Root check
rootCheck(options, alm.config);

alm.config.bin = process.env.BIN;

// Set loglevel
if (alm.config.silent) {
  loglevel = levels.error;
} else if (alm.config.verbose) {
  loglevel = -Infinity;
  Q.longStackSupport = true;
} else if (alm.config.quiet) {
  loglevel = levels.warn;
} else {
  loglevel = levels[alm.config.loglevel] || levels.info;
}
...
while (options.argv.remain.length) {
  ...
  // Alias lookup
  if (alm.abbreviations[command]) {
    command = alm.abbreviations[command].replace(/\s/g, '.');
    break;
  }
  ...
  // Direct lookup
  if (mout.object.has(alm.commands, command)) {
    break;
  }
...
// Execute the command
commandFunc = command && mout.object.get(alm.commands, command);
command = command && command.replace(/\./g, ' ');

// If no command was specified, show alm help
// Do the same if the command is unknown
if (!commandFunc) {
  logger = alm.commands.help();
  command = 'help';
...
// Do the same if the actual command is a group of other commands (e.g.: cache)
  ...
  logger = alm.commands.help(command);
  command = 'help';
...
renderer = cli.getRenderer(command, logger.json, alm.config);

function handleLogger(logger, renderer) {
  logger
    .on('end', function (data) {
      if (!alm.config.silent && !alm.config.quiet) {
        renderer.end(data);
      }
      defer.resolve(data);
    })
    .on('error', function (err) {
      if (command !== 'help' && err.code === 'EREADOPTIONS') {
        logger = alm.commands.help(command);
        renderer = cli.getRenderer('help', logger.json, alm.config);
...
if (alm.config.interactive) {
  var updateNotifier = require('update-notifier');

  // Check for newer version of ano-lm
  var notifier = updateNotifier({pkg: {name: 'ano-lm', version: version}});
