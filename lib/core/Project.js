...
Project.prototype.install = function (decEndpoints, options, config) {
  ...
  return this._analyse()
    .spread(function (json, tree) {
      ...
      // and no ano.json is present in current directory
      if (!that._jsonFile && decEndpoints.length === 0) {
        throw createError('No ano.json present', 'ENOENT');
      }
...
Project.prototype.saveJson = function (forceCreate) {
  ...
  if (!this._jsonFile && !forceCreate) {
    this._logger.warn('no-json', 'No ano.json file to save to, use ano init to create one');
    return Q.resolve();
  }

  file = this._jsonFile || path.join(this._config.cwd, 'ano.json');
...
Project.prototype._readJson = function () {
  ...
  return Q.fcall(function () {
    ...
    if (!assumed) {
      that._jsonFile = path.join(that._config.cwd, deprecated ? deprecated : 'ano.json');
    }
...
Project.prototype._readInstalled = function () {
  ...
  return this._installed = Q.nfcall(glob, '*/.ano.json', {
    cwd: componentsDir,
    dot: true
  })
