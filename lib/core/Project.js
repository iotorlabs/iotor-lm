...
Project.prototype.install = function (decEndpoints, options, config) {
  ...
  return this._analyse()
    .spread(function (json, tree) {
      ...
      // and no library.json is present in current directory
      if (!that._jsonFile && decEndpoints.length === 0) {
        throw createError('No library.json present', 'ENOENT');
      }
    ...
    .spread(function (json, tree, flattened) {
      ...
      if (!names) {
        ...
        that.walkTree(tree, function (node) {
          // We don't know the real source of linked libraries
        ...
        // Add libraries whose names are specified to be updated
        that.walkTree(tree, function (node, name) {
...
Project.prototype.uninstall = function (names, options) {
  ...
  // Analyse the project
  return this._analyse()
  // Fill in the libraries to be uninstalled
    .spread(function (json, tree, flattened) {
        ...
        promise = promise
          .then(function () {
            ...
            if (!dependants.length || that._config.force) {
              libraries[name] = decEndpoint.canonicalDir;
              return;
            }
    ...
    // Remove libraries
    .then(function () {
      return that._removePackages(libraries);
    })
    .fin(function () {
      that._installed = null;
      that._working = false;
...
Project.prototype.saveJson = function (forceCreate) {
  ...
  if (!this._jsonFile && !forceCreate) {
    this._logger.warn('no-json', 'No library.json file to save to, use racoon init to create one');
    return Q.resolve();
  }

  file = this._jsonFile || path.join(this._config.cwd, 'library.json');
...
Project.prototype._readJson = function () {
  ...
  return Q.fcall(function () {
    ...
    if (!assumed) {
      that._jsonFile = path.join(that._config.cwd, deprecated ? deprecated : 'library.json');
    }
...
Project.prototype._removePackages = function (libraries) {
  var that = this;
  var promises = [];

  return scripts.preuninstall(that._config, that._logger, libraries, that._installed, that._json)
    .then(function () {

      mout.object.forOwn(libraries, function (dir, name) {
        var promise;
    ...
    // Run post-uninstall hook before resolving with removed libraries.
    .then(scripts.postuninstall.bind(
      null, that._config, that._logger, libraries, that._installed, that._json))
    // Resolve with removed libraries
    .then(function () {
      return mout.object.filter(libraries, function (dir) {
        return !!dir;
      });
    });
};
...
Project.prototype._restoreNode = function (node, flattened, jsonKey, processed) {
  ...
  mout.object.forOwn(deps, function (value, key) {
    ...
    if (!local) {
      ...
      // Note that linked libraries are interpreted as compatible
