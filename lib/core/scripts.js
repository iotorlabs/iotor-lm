...
var orderByDependencies = function (libraries, installed, json) {
  var ordered = [];
  installed = mout.object.keys(installed);

  var depsSatisfied = function (packageName) {
    return mout.array.difference(mout.object.keys(libraries[packageName].dependencies), installed, ordered).length === 0;
  };

  var depsFromAnoJson = json && json.dependencies ? mout.object.keys(json.dependencies) : [];
  var packageNames = mout.object.keys(libraries);

  //get the list of the libraries that are specified in library.json in that order
  //its nice to maintain that order for users
  var desiredOrder = mout.array.intersection(depsFromAnoJson, packageNames);
  //then add to the end any remaining libraries that werent in library.json
...
var hook = function (action, ordered, config, logger, libraries, installed, json) {
  if (mout.object.keys(libraries).length === 0 || !config.scripts || !config.scripts[action]) {
    return Q();
  }

  var orderedPackages = ordered ? orderByDependencies(libraries, installed, json) : mout.object.keys(libraries);
