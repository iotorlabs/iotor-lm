    ...
    .then(function () {
      ...
      mout.object.forOwn(that._dissected, function (decEndpoint, name) {
        ...
        promise = Q.nfcall(rimraf, dst)
          ...
          .then(function () {
            var metaFile = path.join(dst, '.library.json');

            decEndpoint.canonicalDir = dst;

            // Store additional metadata in library.json
...
Manager.prototype._onFetchSuccess = function (decEndpoint, canonicalDir, pkgMeta, isTargetable) {
  ...
  // If there are no more libraries being fetched,
  // finish the resolve process by dissecting all resolved libraries
...
/**
 * After batch of libraries has been fetched, the results are stored in _resolved hash.
 *
 * {
 ...
 *
 * It stores selected libraries in the this._dissected array.
 *
 * The _dissected array is used later on by install method to move libraries to config.directory.
 */
