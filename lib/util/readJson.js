...
var anoJson = require('ano-json');
var Q = require('q');

// The valid options are the same as ano-json#readFile.
...
function readJson(file, options) {
  ...
  // Read
    return Q.nfcall(anoJson.read, file, options)
    .spread(function (json, jsonFile) {
        var deprecated;

        if (options.logger) {
            var issues = anoJson.getIssues(json);
        ...
        deprecated = jsonFile === 'library.json' ? jsonFile : false;

        return [json, deprecated, false];
    }, function (err) {
      // No json file was found, assume one
        if (err.code === 'ENOENT' && options.assume) {
            return [anoJson.parse(options.assume, options), false, true];
        }
