...
var iotorConfig = require('iotor-config');
...
function readCachedConfig(cwd, overwrites) {
  current = iotorConfig.create(cwd).load(overwrites);

  var config = current.toObject();

  var configstore = new Configstore('iotor-github').all;

  object.mixIn(config, configstore);

  var bin = process.env.BIN || 'iotlm';
