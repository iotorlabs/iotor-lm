...
function StandardRenderer(command, config) {
  ...
  this._config = config || {};
  this._config.bin = this._config.bin || 'iotlm';
...
StandardRenderer.prototype.error = function (err) {
  ...
  this._write(process.stderr, this._config.bin + ' ' + str);
  ...
  if (this._config.verbose || !err.code || err.errno) {
    ...
    // Print iotor version, node version and system info.
    this._write(process.stderr, chalk.yellow('\nSystem info:\n'));
    this._write(process.stderr, 'Ano version: ' + version + '\n');
...
StandardRenderer.prototype._help = function (data) {
  ...
  // For help template to get the real bin
  data.bin = process.env.BIN || this._config.bin;
...
StandardRenderer.prototype._install = function (libraries) {
  var str = '';

  mout.object.forOwn(libraries, function (pkg) {
    var cliTree;
...
StandardRenderer.prototype._update = function (libraries) {
  this._install(libraries);
};
...
StandardRenderer.prototype._link = function (data) {
  ...
  // Print also a tree of the installed libraries
  if (data.installed) {
...
StandardRenderer.prototype._genericLog = function (log) {
  ...
  this._write(stream, this._config.bin + ' ' + str);
};
